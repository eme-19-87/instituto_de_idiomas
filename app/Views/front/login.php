<div class="container-fluid pt-4 pb-5 ps-4">
	<h2 class="text-star mb-5">Iniciar Sesión</h2>
  <div class="row p-3">
  	<div class="col-lg-6 col-md-12 m-auto text-star p-4" style="background: #F9B400; border-radius:15px;">
  		<form class="w-100" method="post">
				<label for="exampleFormControlInput1" class="form-label fw-semibold">Correo electrónico</label>
				<input type="email" class="form-control" id="exampleFormControlInput1" placeholder="jose@gmail.com" id="mail" name="mail">

				<label for="exampleFormControlInput1" class="form-label fw-semibold pt-4">Contraseña</label>
				<input type="password" class="form-control" id="exampleFormControlInput1" placeholder="Abc123456" name="password" id="password" >

				<div class="d-flex pt-4 justify-content-between">
						<button type="button" class="boton-aceptar">
							Registrarse
						</button>

						<button type="button" class="boton-cancelar fw-semibold" >
							Cancelar
						</button>
				</div>
			</form>
		</div>

   
   <div class="col-md-6 m-auto offset-md-0 p-5">
   		<img src="./assets/img/logo-login.svg" class="d-none d-lg-grid w-100" alt="logo-idi">
   </div>

  </div>
  
</div>